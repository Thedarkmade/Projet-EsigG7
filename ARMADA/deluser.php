	...
	if (isset($_SESSION['lvl']) && $_SESSION['lvl']==2){  
        ...
        <?php
				include ("inc/ConnectBdd.inc.php");
				$req_pre = mysqli_prepare($mysqli, 'DELETE FROM `personne` WHERE `login` =?');
				mysqli_stmt_bind_param($req_pre, "s", $log);
				mysqli_stmt_execute($req_pre);
				
			    $req_pre = mysqli_prepare($mysqli, 'DELETE FROM `login` WHERE `login` =?');
				mysqli_stmt_bind_param($req_pre, "s", $log);
				mysqli_stmt_execute($req_pre);
				include ("inc/UconnectBdd.inc.php");
				?>
                 <br/><br/><br/>
                    <div id="intertitre"> 
                   <?php echo htmlspecialchars($log)?> à bien était envoyé dans les limbes.
                    </div>
                 <?php }else{?>   
